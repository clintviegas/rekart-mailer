...
function ProgressBar({ done, total, status }: { done: number; total: number; status: string }) {
  ...
  const color =
    ...
    status === "offer_declined" || status === "request_declined" || status === "no_customer_action"
      ? "bg-red-400"
    :
    "bg-primary";
  return (
...
const STATUS_CONFIG: Record<string, { label: string; cls: string; Icon: React.FC<{ className?: string }> }> = {
  ...
  completed:     { label: "Completed",      cls: "text-emerald-700 bg-emerald-50 border-emerald-200 dark:text-emerald-300 dark:bg-emerald-950/40 dark:border-emerald-800", Icon: CheckCircle2 },
  ...
  offer_declined:{ label: "Offer Declined", cls: "text-red-700 bg-red-50 border-red-200 dark:text-red-300 dark:bg-red-950/40 dark:border-red-800",                       Icon: TrendingDown },
  request_declined: { label: "Request Declined", cls: "text-orange-800 bg-orange-50 border-orange-200 dark:text-orange-300 dark:bg-orange-950/40 dark:border-orange-800", Icon: TrendingDown },
...
function CreateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { mutate: sendNext }            = useSendNextStep();
  const { mutate: sendJourneyStep }     = useSendJourneyStep();

  function reset() {
    setName(""); setEmail(""); setDevice(""); setDeviceBrand("");
    setCondition(""); setConditionOther(""); setEstimatedPriceMin(""); setEstimatedPriceMax("");
  ...
  function buildDynamicData() {
    ...
    if (device)         d.deviceName      = device.trim();
    if (deviceBrand)    d.deviceBrand     = deviceBrand.trim();
    const condResolved = formatDeviceCondition(condition, conditionOther);
    if (condResolved)   d.deviceCondition = condResolved;
    if (estimatedPriceMin.trim()) d.estimatedPriceMin = estimatedPriceMin.trim();
    if (estimatedPriceMax.trim()) d.estimatedPriceMax = estimatedPriceMax.trim();
    if (requestDate)    d.requestDate     = isoYmdToDisplay(requestDate) || requestDate;
  ...
  }, [phase, name, currency, device, deviceBrand, condition, conditionOther, estimatedPriceMin, estimatedPriceMax, requestDate]);

  function validateDeviceDetails(): boolean {
    if (!condition.trim()) {
      toast.error("Item condition is required");
      return false;
    }
    if (condition === "Other" && !conditionOther.trim()) {
      toast.error("Please describe the item condition");
      return false;
    }
    return true;
  }
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await apiClient.post<{ data: { accepted: UploadedFile[]; rejected: string[] } }>(
        "/sell/attachments/upload", formData, { headers: { "Content-Type": "multipart/form-data" } }
      );
  ...
  /** From preview: create journey then immediately send Request Received email */
    ...
    createJourney(
      { customerEmail: email, customerName: name, currency, dynamicData: buildDynamicData(), attachments: attachments.length ? attachments : undefined },
      {
        onSuccess: (newJourney: any) => {
          ...
          sendJourneyStep(
            { id: journeyId, step: "request-received", payload: {} },
            {
              onSuccess: () => { toast.success("Request created — Request Received email sent! ✓"); reset(); onClose(); },
  ...
  return (
    <>
      ...
      >
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Same template as the journey step preview — how the <strong>Request Received</strong> email will look when sent.
              </p>
            </div>
            {draftPreviewLoading ? (
        ...
        {phase !== "preview" && (
        <div className="p-6">
          <h2 className="text-[15px] font-bold text-foreground mb-1">New SELL Request</h2>
          ...
          <form onSubmit={handlePreview} className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="mb-1 block text-[11px] font-semibold text-muted-foreground">Item name</label>
                  <div className="relative">
                    <Package className="absolute left-2.5 top-1/2 -translate-y-1/2 size-3.5 text-muted-foreground" />
                    <Input value={device} onChange={(e) => setDevice(e.target.value)} placeholder="e.g. LG washing machine, 55&quot; TV, iPhone 15 Pro" className="h-9 pl-8" />
                  </div>
                </div>
                <div className="col-span-2">
                  <label className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Apple, Samsung — type any brand"
                    className="h-9"
                  />
                </div>
                <div className="col-span-2">
                  <SelectField
                    ...
                    onChange={(v) => {
                      setCondition(v);
                      if (v !== "Other") setConditionOther("");
                    }}
                    options={SELL_CONDITION_OPTIONS}
                  ...
                  {condition === "Other" ? (
                    <div className="mt-2">
                      ...
                      <Input
                        ...
                        placeholder="e.g. Water damage, replaced battery…"
                        className="h-9"
                      />
                    </div>
                  ) : null}
            ...
            {/* ── Pricing & Date ────────────────────────────── */}
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Pricing & Date</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-[11px] font-semibold text-muted-foreground">Min price (estimate)</label>
                  <Input
                    {...MONEY_INPUT_PROPS}
                    placeholder="e.g. 400"
                    value={estimatedPriceMin} onChange={(e) => setEstimatedPriceMin(e.target.value)}
                    className="h-9"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[11px] font-semibold text-muted-foreground">Max price (estimate)</label>
                  <Input
                    {...MONEY_INPUT_PROPS}
                    placeholder="e.g. 600"
                    value={estimatedPriceMax} onChange={(e) => setEstimatedPriceMax(e.target.value)}
                    className="h-9"
                  />
                </div>
                <div className="col-span-2">
...
function RequestRow({ journey, onClick }: { journey: SellRequestJourney; onClick: () => void }) {
  const done  = journey.completedSteps.length;
  const total = JOURNEY_WORKFLOW_STEPS.length;
  const nextLabel = !["completed", "cancelled", "offer_declined", "request_declined", "no_customer_action"].includes(journey.status)
    ? JOURNEY_STEP_LABELS[journey.currentStep as keyof typeof JOURNEY_STEP_LABELS]
    : null;

  const highlight = getJourneyMoneyHighlight(journey.dynamicData as Record<string, unknown>);
  ...
  return (
    ...
    >
      ...
      <td className="hidden px-3 py-3.5 md:table-cell">
        {journey.dynamicData?.deviceName ? (
          <span className="inline-flex items-center gap-1.5 text-[12px] text-muted-foreground">
            <Package className="size-3.5 shrink-0" />
            {String(journey.dynamicData.deviceName)}
          </span>
        ) : (
        ...
        <div className="flex flex-col gap-1">
          ...
          {journey.offerExpiresAt && journey.status === "active" &&
            ...
            <span className="inline-flex items-center gap-0.5 rounded-full bg-blue-500/10 px-1.5 py-0.5 text-[9px] font-semibold text-blue-600 ring-1 ring-blue-400/30">
              Expires {formatRelativeExpiry(journey.offerExpiresAt)}
            </span>
          )}
        </div>
      </td>
...
const PIPELINE_STEPS = [
  { key: "request-received",    label: "Request\u00a0Received",    short: "Request" },
  { key: "pickup-scheduled",    label: "Pickup\u00a0Scheduled",    short: "Pickup"  },
  { key: "inspection-underway", label: "Inspection\u00a0Underway", short: "Inspect" },
  { key: "offer-ready",         label: "Offer\u00a0Ready",         short: "Offer"   },
  { key: "payment-sent",        label: "Payment\u00a0Sent",        short: "Payment" },
] as const;
...
const PIPELINE_COLORS: Record<PipelineStepKey, { bg: string; text: string; ring: string; activeBg: string; activeText: string }> = {
  "request-received":    { bg: "bg-sky-50",    text: "text-sky-700",    ring: "ring-sky-200",    activeBg: "bg-sky-500",    activeText: "text-white" },
  "pickup-scheduled":    { bg: "bg-violet-50", text: "text-violet-700", ring: "ring-violet-200", activeBg: "bg-violet-500", activeText: "text-white" },
  "inspection-underway": { bg: "bg-amber-50",  text: "text-amber-700",  ring: "ring-amber-200",  activeBg: "bg-amber-500",  activeText: "text-white" },
  "offer-ready":         { bg: "bg-orange-50", text: "text-orange-700", ring: "ring-orange-200", activeBg: "bg-orange-500", activeText: "text-white" },
  "payment-sent":        { bg: "bg-emerald-50",text: "text-emerald-700",ring: "ring-emerald-200",activeBg: "bg-emerald-500",activeText: "text-white" },
};
...
const TABS: TabDef[] = [
  ...
  { key: "reship",          label: "Reship" },
  { key: "offer_declined",  label: "Offer ↓" },
...
function tabFromSearchParams(sp: Pick<URLSearchParams, "get">): FilterTab {
  ...
  if (truthyQuery(sp.get("revisedOffers"))) return "revised";
  if (truthyQuery(sp.get("closedByReship"))) return "reship";
  return "all";
}

/** Build /dashboard/sell query preserving optional insight filters */
function buildSellListPath(
  tab: FilterTab,
  insight: { completedStep: string | null; revisedOffers: boolean; closedByReship: boolean },
): string {
  const sp = new URLSearchParams();
  if (tab !== "all") sp.set("status", tab);
  /** These tabs encode their own full slice — no stacking of insight params. */
  if (tab !== "reship" && tab !== "revised" && tab !== "reminder_due") {
    if (insight.completedStep) sp.set("completedStep", insight.completedStep);
    if (insight.revisedOffers) sp.set("revisedOffers", "1");
    if (insight.closedByReship) sp.set("closedByReship", "1");
  }
  const q = sp.toString();
  return q ? `/dashboard/sell?${q}` : "/dashboard/sell";
}
...
export default function SellRequestsPage() {
  ...
  const revisedOffersFilter  = truthyQuery(searchParams.get("revisedOffers"));
  const closedByReshipFilter = truthyQuery(searchParams.get("closedByReship"));

  const hasInsightFilter =
    !!completedStepParam || revisedOffersFilter || closedByReshipFilter;
  ...
  const clearInsightFilters = useCallback(() => {
    router.replace(
      buildSellListPath(tab, {
        completedStep: null,
        revisedOffers: false,
        closedByReship: false,
      }),
      { scroll: false },
    );
  }, [router, tab]);

  const { data: statsData, isLoading: statsLoading, refetch: refetchStats } = useSellStats();

  const isReshipTab    = tab === "reship";
  const isRevisedTab   = tab === "revised";
  ...
  useEffect(() => { setPage(1); }, [tab, debouncedSearch, completedStepParam, revisedOffersFilter, closedByReshipFilter, pipelineStep]);

  const activeStatus =
    tab === "all" || isRevisedTab || isReminderTab ? undefined
    : isReshipTab ? "completed" : tab;

  const { data, isLoading, refetch, isFetching } = useJourneys({
    search: debouncedSearch || undefined,
    status: pipelineStep ? "active" : activeStatus,
    completedStep: isReshipTab || isRevisedTab || isReminderTab || pipelineStep ? undefined : (completedStepParam ?? undefined),
    revisedOffers:
      isRevisedTab ? true : isReshipTab ? undefined : (revisedOffersFilter || undefined),
    closedByReship: isReshipTab ? true : (closedByReshipFilter || undefined),
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      await sellService.exportJourneysCsv({
        status: activeStatus,
        search: debouncedSearch || undefined,
      });
    } catch {
  ...
  const journeys: SellRequestJourney[] = data?.data ?? [];
  ...
  const badgeCounts = useMemo(() => {
    ...
    return {
      ...
      revised: statsData?.revisedOfferJourneys ?? 0,
      reship: statsData?.journeysClosedByReship ?? 0,
      offer_declined: bs?.offer_declined ?? 0,
      request_declined: bs?.request_declined ?? 0,
  ...
  const insightForTabs = useMemo(
    () => ({
      ...
      revisedOffers: revisedOffersFilter,
      closedByReship: closedByReshipFilter,
    }),
    [completedStepParam, revisedOffersFilter, closedByReshipFilter],
  );
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 border-b border-border bg-card px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-[15px] font-bold text-foreground">SELL Requests</h1>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Track and manage customer device sell journeys
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Link
        ...
        <div className="mt-2.5 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter journeys by status, revised offers, or reship"
            >
              {TABS.map(({ key, label, urgent }) => {
                const href = buildSellListPath(key, insightForTabs);
        ...
        {(search.trim() || hasInsightFilter) ? (
          <p className="text-[10px] text-muted-foreground mt-2 pl-0.5">
            {search.trim() ? (
              <>Search narrows the table. Tab counts are still workspace-wide (same source as Overview). <strong>Revised</strong> tab = 2nd+ offer round. <strong>Reship</strong> = completed after return-shipment email.</>
            ) : (
              <>Funnel / insight filters are on the URL. Tab counts stay full-workspace; use the <strong>Revised</strong> or <strong>Reship</strong> tab for those lists.</>
            )}
          </p>
        ) : (
          <p className="text-[10px] text-muted-foreground mt-2 pl-0.5">
            Tab counts match Overview. <strong>Completed</strong> is every finished journey. <strong>Revised</strong> counts journeys with an extra offer round (badge = journeys, not emails — extra sends are in Overview). <strong>Reship</strong> is the subset closed via device return email.
          </p>
        )}
      </div>
            ...
            <p className="text-[11px] text-muted-foreground">
              ...
              {completedStepParam ? (
                <>
                  ...
                  <strong>
                    {JOURNEY_STEP_LABELS[completedStepParam as keyof typeof JOURNEY_STEP_LABELS] ??
                      completedStepParam}
                  </strong>
                  .
                </>
              ...
              {revisedOffersFilter ? (
                <span>
                  {hasInsightFilter && completedStepParam ? " " : ""}
                  Journeys with a <strong>revised offer</strong> (2nd+ offer round).
                </span>
              ) : null}
              {closedByReshipFilter ? (
                <span>
                  {(completedStepParam || revisedOffersFilter) ? " " : ""}
                  Journeys <strong>closed after device reship</strong>.
                </span>
              ) : null}
            </p>
            <button
              ...
              <p className="mt-1 text-[11px] text-muted-foreground max-w-xs">
                {search
                  ...
                  : hasInsightFilter
                    ...
                    : "Create your first SELL request to start a customer device sell journey."}
              </p>
            </div>
            {!search && (
            ...
            <thead className="sticky top-0 z-10 border-b border-border bg-card">
              <tr>
                {[
                  ...
                  { h: "Amount",      cls: "hidden sm:table-cell" },
              ...
              <AnimatePresence mode="popLayout">
                {journeys.map((j) => (
                  <RequestRow
                    key={j._id}
                    journey={j}
                    onClick={() => router.push(`/dashboard/sell/${j._id}`)}
                  />
                ))}
              </AnimatePresence>
            </tbody>
