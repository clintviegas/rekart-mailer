...
import { useSellStats } from "@/hooks/use-sell-templates";
import {
...
import type { SellRequestJourney, SellJourneyAction, JourneyActionType, JourneyStatus } from "@/types/sell";
import {
...
function FunnelBar({
  ...
  const label = journeyStepLabel(step);

  const STEP_COLOR: Record<string, string> = {
    "request-received":    "bg-blue-500",
    "pickup-scheduled":    "bg-violet-500",
    "inspection-underway": "bg-amber-500",
    "offer-ready":         "bg-orange-500",
    "payment-sent":        "bg-emerald-500",
    "completed":           "bg-teal-600",
    [DEVICE_RESHIP_STEP]:  "bg-violet-600",
  };
...
const STATUS_STYLE: Record<string, { dot: string; text: string; bg: string }> = {
  ...
  offer_declined:{ dot: "bg-red-500",     text: "text-red-700 dark:text-red-300",      bg: "bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800" },
  ...
  reship:        { dot: "bg-violet-500",  text: "text-violet-800 dark:text-violet-300", bg: "bg-violet-50 dark:bg-violet-950/35 border-violet-200 dark:border-violet-800" },
...
const ACTION_META: Partial<Record<JourneyActionType, { icon: string; label: string }>> = {
  ...
  offer_accepted:       { icon: "✅", label: "Offer Accepted" },
  offer_declined:       { icon: "❌", label: "Offer Declined" },
...
export function SellOverviewClient() {
  const router = useRouter();
  const { data: stats, isLoading, refetch, isFetching } = useSellStats();
  ...
  const recentJourneys = (stats?.recentJourneys ?? []) as SellRequestJourney[];
  const recentActions  = (stats?.recentActions  ?? []) as SellJourneyAction[];
  const revisedOfferJourneys = stats?.revisedOfferJourneys ?? 0;
  const totalRevisedOfferEmails = stats?.totalRevisedOfferEmails ?? 0;
  const journeysClosedByReship = stats?.journeysClosedByReship ?? 0;
  const maxStepCount  = Math.max(
    ...JOURNEY_WORKFLOW_STEPS.map((s) => byStep[s] ?? 0),
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 border-b border-border bg-card px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[15px] font-bold text-foreground">SELL Overview</h1>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Real-time summary of all sell request journeys and email activity
            </p>
          </div>
      ...
      >
        ...
        {/* ── KPI row ─────────────────────────────────────────────────── */}
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-7">
          {isLoading ? (
            ...
            <>
              <StatCard delay={0}    label="Total Requests"   value={total}              icon={Users}        color="bg-blue-50 dark:bg-blue-950/40"    iconColor="text-blue-600 dark:text-blue-400" href="/dashboard/sell" />
              <StatCard delay={0.05} label="Active"           value={byStatus.active}    icon={Activity}     color="bg-violet-50 dark:bg-violet-950/40" iconColor="text-violet-600 dark:text-violet-400" href="/dashboard/sell?status=active" />
              <StatCard delay={0.1}  label="Completed"        value={byStatus.completed} icon={CheckCircle2} color="bg-emerald-50 dark:bg-emerald-950/40" iconColor="text-emerald-600 dark:text-emerald-400" href="/dashboard/sell?status=completed" />
              <StatCard delay={0.15} label="Offer Declined"   value={byStatus.offer_declined} icon={XCircle} color="bg-red-50 dark:bg-red-950/40"       iconColor="text-red-600 dark:text-red-400" href="/dashboard/sell?status=offer_declined" />
              <StatCard delay={0.175} label="Request Declined" value={byStatus.request_declined} icon={XCircle} color="bg-orange-50 dark:bg-orange-950/40" iconColor="text-orange-600 dark:text-orange-400" href="/dashboard/sell?status=request_declined" />
              <StatCard delay={0.2}  label="Acceptance Rate"  value={`${acceptRate}%`}   icon={Percent}      color="bg-amber-50 dark:bg-amber-950/40"   iconColor="text-amber-600 dark:text-amber-400"
                sub={`${byStep["payment-sent"] ?? 0} of ${byStep["offer-ready"] ?? 0} offers`}
                href="/dashboard/sell?completedStep=offer-ready" />
              <StatCard delay={0.25} label="Total Revenue"    value={fmtCurrency(revenue)} icon={Banknote}   color="bg-teal-50 dark:bg-teal-950/40"     iconColor="text-teal-600 dark:text-teal-400"
                sub="from completed journeys"
                href="/dashboard/sell?status=completed" />
            </>
          )}
        </div>

        {/* Negotiation & reship KPIs */}
        <div>
          <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground mb-3">
            Revised offers & reship
          </p>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
            {isLoading ? (
              ...
              <>
                <StatCard
                  delay={0.05}
                  label="Journeys with revised offer"
                  value={revisedOfferJourneys}
                  sub="At least one extra offer-ready round"
                  icon={RefreshCw}
                  color="bg-orange-50 dark:bg-orange-950/40"
                  iconColor="text-orange-600 dark:text-orange-400"
                  href="/dashboard/sell?status=revised"
                />
                <StatCard
                  delay={0.1}
                  label="Revised offer emails (total)"
                  value={totalRevisedOfferEmails}
                  sub="Extra offer sends workspace-wide"
                  icon={MailCheck}
                ...
                <StatCard
                  delay={0.15}
                  label="Closed via return shipment"
                  value={journeysClosedByReship}
                  sub="Completed after reship email"
                  icon={Package}
                  color="bg-violet-50 dark:bg-violet-950/40"
                  iconColor="text-violet-600 dark:text-violet-400"
                  href="/dashboard/sell?status=reship"
                />
              </>
            )}
          </div>
        ...
        {/* ── Middle row: Funnel + Status breakdown ─────────────────────── */}
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          >
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {JOURNEY_WORKFLOW_STEPS.map((step) => (
                  <FunnelBar
                    key={step}
                    step={step}
                    count={byStep[step] ?? 0}
                    max={maxStepCount}
                    href={`/dashboard/sell?completedStep=${encodeURIComponent(step)}`}
                  />
                ))}
                <FunnelBar
                  step={DEVICE_RESHIP_STEP}
                  count={journeysClosedByReship}
                  max={maxStepCount}
                  href="/dashboard/sell?status=reship"
                />
              </div>
          ...
          >
            ...
            {isLoading ? (
              ...
              <div className="space-y-2.5">
                ...
                {(["active", "completed", "reship", "offer_declined", "request_declined", "no_customer_action", "cancelled"] as const).map((s) => {
                  const count =
                    s === "reship" ? journeysClosedByReship : (byStatus[s] ?? 0);
                  ...
                  const label =
                    s === "offer_declined"
                      ...
                      : s === "no_customer_action"
                        ? "No Response"
                      : s === "reship"
                        ? "Reship (device return)"
                        : s.charAt(0).toUpperCase() + s.slice(1);
                  const style = STATUS_STYLE[s];
                  const href =
                    s === "reship" ? "/dashboard/sell?status=reship" : `/dashboard/sell?status=${s}`;
                  return (
                    <Link
                      key={s}
                      href={href}
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Latest 5 sell request journeys</p>
              </div>
              <button
                type="button"
                onClick={() => router.push("/dashboard/sell")}
                ...
                recentActions.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={!journeyId}
                      onClick={() => journeyId && router.push(`/dashboard/sell/${journeyId}`)}
                      className={cn(
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground truncate mt-0.5">
                          {JOURNEY_STEP_LABELS[(a.step as keyof typeof JOURNEY_STEP_LABELS)] ?? a.step}
                        </p>
                      </div>
        ...
        >
          {[
            { label: "New Request",    sub: "Start a sell journey",       href: "/dashboard/sell",         icon: Users,        color: "text-primary" },
            { label: "Email Design",   sub: "Branding & design settings", href: "/dashboard/sell/design",  icon: TrendingUp,   color: "text-violet-600 dark:text-violet-400" },
            { label: "Active Journeys",sub: `${byStatus.active} in progress`, href: "/dashboard/sell?status=active", icon: Activity, color: "text-blue-600 dark:text-blue-400" },
            { label: "Completed",      sub: `${byStatus.completed} journeys done`, href: "/dashboard/sell?status=completed", icon: CheckCircle2, color: "text-emerald-600 dark:text-emerald-400" },
