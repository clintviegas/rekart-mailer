...
import { useRepairAnalytics } from "@/hooks/use-repair-journeys";
import type { JourneyAnalytics, JourneyFunnelStep } from "@/types/repair";
...
const ACTION_LABELS: Record<string, string> = {
  offer_accepted:              "Quote Accepted",
  quote_accepted:              "Quote Accepted",
  quote_declined:              "Quote Declined",
  request_received_accepted:   "Request Confirmed",
  request_received_declined:   "Booking Declined",
...
// ── Main component ─────────────────────────────────────────────────────────────
export function RepairAnalyticsClient() {
  const { data, isLoading, isError, refetch, isFetching } = useRepairAnalytics();
  ...
  const kpis = useMemo(() => {
    ...
    const quoteReadyCount = data.funnel.find((f) => f.step === "quote-ready")?.count ?? 0;
    const repairStartedCount = data.funnel.find((f) => f.step === "repair-in-progress")?.count ?? 0;
    const acceptanceRate = quoteReadyCount > 0 ? Math.round((repairStartedCount / quoteReadyCount) * 100) : 0;
  ...
  function exportAllData() {
    ...
    downloadCsv(rows, `repair-analytics-${new Date().toISOString().slice(0, 10)}.csv`);
  }
  ...
  return (
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Repair Analytics</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        {/* Revenue by currency */}
        <Section title="Revenue by Currency" sub="Device Returned journeys only">
