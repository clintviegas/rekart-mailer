...
import { useRepairStats } from "@/hooks/use-repair-journeys";
import {
  REPAIR_JOURNEY_WORKFLOW_STEPS,
  REPAIR_JOURNEY_STEP_LABELS,
  repairJourneyStepLabel,
} from "@/types/repair";
import type { RepairRequestJourney, RepairJourneyAction, JourneyActionType, JourneyStatus } from "@/types/repair";
import {
...
function FunnelBar({
  ...
  const label = repairJourneyStepLabel(step);
  ...
  const STEP_COLOR: Record<string, string> = {
    ...
    "repair-in-progress":        "bg-emerald-500",
...
const ACTION_META: Partial<Record<JourneyActionType, { icon: string; label: string }>> = {
  ...
  offer_accepted:       { icon: "✅", label: "Quote Accepted" },
  quote_declined:       { icon: "❌", label: "Quote Declined" },
  request_received_accepted: { icon: "✅", label: "Request Confirmed" },
  request_received_declined: { icon: "✖️", label: "Booking Declined" },
...
export function RepairOverviewClient() {
  const router = useRouter();
  const { data: stats, isLoading, refetch, isFetching } = useRepairStats();
  ...
  const recentJourneys = (stats?.recentJourneys ?? []) as RepairRequestJourney[];
  const recentActions  = (stats?.recentActions  ?? []) as RepairJourneyAction[];
  ...
  const maxStepCount  = Math.max(
    ...REPAIR_JOURNEY_WORKFLOW_STEPS.map((s) => byStep[s] ?? 0),
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 border-b border-border bg-card px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[15px] font-bold text-foreground">Repair Overview</h1>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Real-time summary of all repair request journeys and email activity
            </p>
          </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-7">
          {isLoading ? (
            ...
            <>
              <StatCard delay={0}    label="Total Requests"   value={total}              icon={Users}        color="bg-blue-50 dark:bg-blue-950/40"    iconColor="text-blue-600 dark:text-blue-400" href="/dashboard/repair" />
              <StatCard delay={0.05} label="Active"           value={byStatus.active}    icon={Activity}     color="bg-violet-50 dark:bg-violet-950/40" iconColor="text-violet-600 dark:text-violet-400" href="/dashboard/repair?status=active" />
              <StatCard delay={0.1}  label="Device Returned"        value={byStatus.completed} icon={CheckCircle2} color="bg-emerald-50 dark:bg-emerald-950/40" iconColor="text-emerald-600 dark:text-emerald-400" href="/dashboard/repair?status=completed" />
              <StatCard delay={0.15} label="Quote Declined"   value={byStatus.quote_declined} icon={XCircle} color="bg-red-50 dark:bg-red-950/40"       iconColor="text-red-600 dark:text-red-400" href="/dashboard/repair?status=quote_declined" />
              <StatCard delay={0.175} label="Booking Declined" value={byStatus.booking_declined} icon={XCircle} color="bg-orange-50 dark:bg-orange-950/40" iconColor="text-orange-600 dark:text-orange-400" href="/dashboard/repair?status=booking_declined" />
              <StatCard delay={0.2}  label="Acceptance Rate"  value={`${acceptRate}%`}   icon={Percent}      color="bg-amber-50 dark:bg-amber-950/40"   iconColor="text-amber-600 dark:text-amber-400"
                sub={`${byStep["repair-in-progress"] ?? 0} of ${byStep["quote-ready"] ?? 0} quotes`}
                href="/dashboard/repair?completedStep=quote-ready" />
              <StatCard delay={0.25} label="Total Revenue"    value={fmtCurrency(revenue)} icon={Banknote}   color="bg-teal-50 dark:bg-teal-950/40"     iconColor="text-teal-600 dark:text-teal-400"
                ...
                href="/dashboard/repair?status=completed" />
            </>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-2">
            {isLoading ? (
              ...
              <>
                <StatCard
                  delay={0.05}
                  label="Journeys with revised quote"
                  value={revisedQuoteJourneys}
                  sub="At least one extra quote-ready round"
                  icon={RefreshCw}
                  color="bg-orange-50 dark:bg-orange-950/40"
                  iconColor="text-orange-600 dark:text-orange-400"
                  href="/dashboard/repair?revisedQuotes=true"
                />
                <StatCard
                  delay={0.1}
                  label="Revised quote emails (total)"
                  value={totalRevisedQuoteEmails}
                  sub="Extra quote sends workspace-wide"
                  icon={MailCheck}
        ...
        <div className="grid gap-4 lg:grid-cols-5">
          ...
          >
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {REPAIR_JOURNEY_WORKFLOW_STEPS.map((step) => (
                  <FunnelBar
                    key={step}
                    step={step}
                    count={byStep[step] ?? 0}
                    max={maxStepCount}
                    href={`/dashboard/repair?completedStep=${encodeURIComponent(step)}`}
                  />
                ))}
              </div>
            )}
          ...
          >
            ...
            {isLoading ? (
              ...
              <div className="space-y-2.5">
                ...
                {(["active", "completed", "quote_declined", "booking_declined", "no_customer_action", "cancelled"] as const).map((s) => {
                  ...
                  const href = `/dashboard/repair?status=${s}`;
                  return (
                    <Link
                      key={s}
                      href={href}
          ...
          >
            <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">Latest 5 repair request journeys</p>
              </div>
              <button
                type="button"
                onClick={() => router.push("/dashboard/repair")}
                ...
                recentActions.map((a) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={!journeyId}
                      onClick={() => journeyId && router.push(`/dashboard/repair/${journeyId}`)}
                      className={cn(
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground truncate mt-0.5">
                          {REPAIR_JOURNEY_STEP_LABELS[(a.step as keyof typeof REPAIR_JOURNEY_STEP_LABELS)] ?? a.step}
                        </p>
                      </div>
        ...
        >
          {[
            { label: "New Request",    sub: "Start a repair journey",       href: "/dashboard/repair",         icon: Users,        color: "text-primary" },
            { label: "Email Design",   sub: "Branding & design settings", href: "/dashboard/repair/design",  icon: TrendingUp,   color: "text-violet-600 dark:text-violet-400" },
            { label: "Active Journeys",sub: `${byStatus.active} in progress`, href: "/dashboard/repair?status=active", icon: Activity, color: "text-blue-600 dark:text-blue-400" },
            { label: "Device Returned",      sub: `${byStatus.completed} journeys done`, href: "/dashboard/repair?status=completed", icon: CheckCircle2, color: "text-emerald-600 dark:text-emerald-400" },
