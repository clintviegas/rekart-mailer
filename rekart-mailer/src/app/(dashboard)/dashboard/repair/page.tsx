...
import {
  useRepairJourneys,
  useCreateRepairJourney,
  useSendNextRepairStep,
...
import {
  SELL_EMAIL_PREVIEW_FRAME_WRAP_CLASS as REPAIR_EMAIL_PREVIEW_FRAME_WRAP_CLASS,
  SELL_EMAIL_PREVIEW_IFRAME_CLASS as REPAIR_EMAIL_PREVIEW_IFRAME_CLASS,
...
function CreateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { mutate: createJourney }       = useCreateRepairJourney();
  const { mutate: sendNext }            = useSendNextRepairStep();
  const { mutate: sendJourneyStep }     = useSendRepairJourneyStep();
  ...
  function buildDynamicData() {
    ...
    if (issueDescription.trim()) d.issueDescription = issueDescription.trim();
  ...
  }, [phase, name, currency, device, deviceBrand, issueDescription, requestDate]);
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await apiClient.post<{ data: { accepted: UploadedFile[]; rejected: string[] } }>(
        "/repair/attachments/upload", formData, { headers: { "Content-Type": "multipart/form-data" } }
      );
  ...
  function handlePreview(e: React.FormEvent) {
    ...
    if (!issueDescription.trim()) { toast.error("Issue description is required"); return; }
    setPhase("preview");
  }
    ...
    createJourney(
      { customerEmail: email, customerName: name, currency, dynamicData: buildDynamicData(), attachments: attachments.length ? attachments : undefined },
      {
        onSuccess: (newJourney: any) => {
          ...
          sendJourneyStep(
            { id: journeyId, step: "booking-confirmed", payload: {} },
            {
              onSuccess: () => { toast.success("Request created — Booking Confirmed email sent! ✓"); reset(); onClose(); },
  ...
  return (
    <>
      ...
      >
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Same template as the journey step preview — how the <strong>Booking Confirmed</strong> email will look when sent.
              </p>
            </div>
            {draftPreviewLoading ? (
              ...
              <div className={REPAIR_EMAIL_PREVIEW_FRAME_WRAP_CLASS}>
                {draftPreviewSubject ? (
                ...
                <iframe
                  ...
                  className={REPAIR_EMAIL_PREVIEW_IFRAME_CLASS}
                />
              </div>
            ) : (
        ...
        {phase !== "preview" && (
        <div className="p-6">
          <h2 className="text-[15px] font-bold text-foreground mb-1">New Repair Request</h2>
          ...
          <form onSubmit={handlePreview} className="space-y-4">
            ...
            {/* ── Issue & Date ────────────────────────────── */}
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Issue & Date</p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="mb-1 block text-[11px] font-semibold text-muted-foreground">Issue description *</label>
                  <textarea
                    value={issueDescription}
                    onChange={(e) => setIssueDescription(e.target.value)}
                    placeholder="e.g. Screen cracked, battery not charging, washing machine not spinning…"
                    rows={3}
                    className="w-full rounded-md border border-input bg-background px-3 py-2 text-[13px] text-foreground focus:outline-none focus:ring-1 focus:ring-ring resize-y min-h-[72px]"
                  />
                </div>
                <div className="col-span-2">
                  <label className="mb-1 block text-[11px] font-semibold text-muted-foreground">Booking Date</label>
                  <Input
                    type="date"
...
function RequestRow({ journey, onClick }: { journey: RepairRequestJourney; onClick: () => void }) {
  const done  = journey.completedSteps.length;
  const total = REPAIR_JOURNEY_WORKFLOW_STEPS.length;
  const nextLabel = !["completed", "cancelled", "quote_declined", "booking_declined", "no_customer_action"].includes(journey.status)
    ? REPAIR_JOURNEY_STEP_LABELS[journey.currentStep as keyof typeof REPAIR_JOURNEY_STEP_LABELS]
    : null;
...
const PIPELINE_STEPS = [
  { key: "booking-confirmed", label: "Booking\u00a0Confirmed", short: "Booking" },
  { key: "pickup-scheduled", label: "Pickup\u00a0Scheduled", short: "Pickup" },
  { key: "device-received", label: "Device\u00a0Received", short: "Received" },
  { key: "diagnosing", label: "Diagnosing", short: "Diagnose" },
  { key: "quote-ready", label: "Quote\u00a0Ready", short: "Quote" },
  { key: "repair-in-progress", label: "Repair\u00a0In\u00a0Progress", short: "Repair" },
  { key: "device-ready", label: "Device\u00a0Ready", short: "Ready" },
  { key: "device-returned", label: "Device\u00a0Returned", short: "Returned" },
] as const;
...
const PIPELINE_COLORS: Record<PipelineStepKey, { bg: string; text: string; ring: string; activeBg: string; activeText: string }> = {
  "booking-confirmed":    { bg: "bg-sky-50",    text: "text-sky-700",    ring: "ring-sky-200",    activeBg: "bg-sky-500",    activeText: "text-white" },
  "pickup-scheduled":     { bg: "bg-violet-50", text: "text-violet-700", ring: "ring-violet-200", activeBg: "bg-violet-500", activeText: "text-white" },
  "device-received":      { bg: "bg-indigo-50", text: "text-indigo-700", ring: "ring-indigo-200", activeBg: "bg-indigo-500", activeText: "text-white" },
  "diagnosing":           { bg: "bg-amber-50",  text: "text-amber-700",  ring: "ring-amber-200",  activeBg: "bg-amber-500",  activeText: "text-white" },
  "quote-ready":          { bg: "bg-orange-50", text: "text-orange-700", ring: "ring-orange-200", activeBg: "bg-orange-500", activeText: "text-white" },
  "repair-in-progress":   { bg: "bg-emerald-50",text: "text-emerald-700",ring: "ring-emerald-200",activeBg: "bg-emerald-500",activeText: "text-white" },
  "device-ready":         { bg: "bg-teal-50",   text: "text-teal-700",   ring: "ring-teal-200",   activeBg: "bg-teal-500",   activeText: "text-white" },
  "device-returned":      { bg: "bg-slate-100", text: "text-slate-700",  ring: "ring-slate-200",  activeBg: "bg-slate-600",  activeText: "text-white" },
};
...
const TABS: TabDef[] = [
  ...
  { key: "completed",       label: "Device Returned" },
  { key: "reminder_due",    label: "Reminder Due", urgent: true },
  { key: "revised",         label: "Revised" },
  { key: "quote_declined",  label: "Quote ↓" },
  { key: "booking_declined", label: "Booking ↓" },
...
function tabFromSearchParams(sp: Pick<URLSearchParams, "get">): FilterTab {
  ...
  if (truthyQuery(sp.get("revisedQuotes"))) return "revised";
  return "all";
}

/** Build /dashboard/repair query preserving optional insight filters */
function buildRepairListPath(
  tab: FilterTab,
  insight: { completedStep: string | null; revisedQuotes: boolean },
): string {
  const sp = new URLSearchParams();
  if (tab !== "all") sp.set("status", tab);
  if (tab !== "revised" && tab !== "reminder_due") {
    if (insight.completedStep) sp.set("completedStep", insight.completedStep);
    if (insight.revisedQuotes) sp.set("revisedQuotes", "1");
  }
  const q = sp.toString();
  return q ? `/dashboard/repair?${q}` : "/dashboard/repair";
}
...
export default function RepairRequestsPage() {
  ...
  const revisedQuotesFilter  = truthyQuery(searchParams.get("revisedQuotes"));
  const hasInsightFilter =
    !!completedStepParam || revisedQuotesFilter;
  ...
  const { data: statsData, isLoading: statsLoading, refetch: refetchStats } = useRepairStats();
  ...
  useEffect(() => { setPage(1); }, [tab, debouncedSearch, completedStepParam, revisedQuotesFilter, pipelineStep]);

  const activeStatus =
    tab === "all" || isRevisedTab || isReminderTab ? undefined : tab;

  const { data, isLoading, refetch, isFetching } = useRepairJourneys({
    search: debouncedSearch || undefined,
    status: pipelineStep ? "active" : activeStatus,
    completedStep: isRevisedTab || isReminderTab || pipelineStep ? undefined : (completedStepParam ?? undefined),
    revisedQuotes: isRevisedTab ? true : (revisedQuotesFilter || undefined),
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      await repairService.exportJourneysCsv({
        status: activeStatus,
        search: debouncedSearch || undefined,
      });
    } catch {
  ...
  const journeys: RepairRequestJourney[] = data?.data ?? [];
  ...
  const badgeCounts = useMemo(() => {
    ...
    return {
      ...
      revised: statsData?.revisedQuoteJourneys ?? 0,
      quote_declined: bs?.quote_declined ?? 0,
      booking_declined: bs?.booking_declined ?? 0,
  ...
  const insightForTabs = useMemo(
    () => ({
      ...
      revisedQuotes: revisedQuotesFilter,
    }),
    [completedStepParam, revisedQuotesFilter],
  );
    ...
    <div className="flex h-full flex-col overflow-hidden bg-background">
      ...
      <div className="shrink-0 border-b border-border bg-card px-6 py-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-[15px] font-bold text-foreground">Repair Requests</h1>
            <p className="text-[11px] text-muted-foreground mt-0.5">
              Track and manage customer device repair journeys
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Link
        ...
        <div className="mt-2.5 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:gap-3">
            ...
            <div
              ...
              role="tablist"
              aria-label="Filter journeys by status or revised quotes"
            >
              {TABS.map(({ key, label, urgent }) => {
                const href = buildRepairListPath(key, insightForTabs);
        ...
        {(search.trim() || hasInsightFilter) ? (
          <p className="text-[10px] text-muted-foreground mt-2 pl-0.5">
            {search.trim() ? (
              <>Search narrows the table. Tab counts are still workspace-wide (same source as Overview). <strong>Revised</strong> tab = 2nd+ quote round.</>
            ) : (
              <>Funnel / insight filters are on the URL. Tab counts stay full-workspace; use the <strong>Revised</strong> tab for those lists.</>
            )}
          </p>
        ) : (
          <p className="text-[10px] text-muted-foreground mt-2 pl-0.5">
            Tab counts match Overview. <strong>Device Returned</strong> is every finished journey. <strong>Revised</strong> counts journeys with an extra quote round (badge = journeys, not emails — extra sends are in Overview).
          </p>
        )}
      </div>
            ...
            <p className="text-[11px] text-muted-foreground">
              ...
              {completedStepParam ? (
                <>
                  ...
                  <strong>
                    {REPAIR_JOURNEY_STEP_LABELS[completedStepParam as keyof typeof REPAIR_JOURNEY_STEP_LABELS] ??
                      completedStepParam}
                  </strong>
                  .
                </>
              ...
              {revisedQuotesFilter ? (
                <span>
                  ...
                  Journeys with a <strong>revised quote</strong> (2nd+ quote round).
                </span>
              ) : null}
            </p>
            <button
              ...
              <p className="mt-1 text-[11px] text-muted-foreground max-w-xs">
                {search
                  ...
                  : hasInsightFilter
                    ...
                    : "Create your first Repair request to start a customer device repair journey."}
              </p>
            </div>
            {!search && (
              ...
              <AnimatePresence mode="popLayout">
                {journeys.map((j) => (
                  <RequestRow
                    key={j._id}
                    journey={j}
                    onClick={() => router.push(`/dashboard/repair/${j._id}`)}
                  />
                ))}
              </AnimatePresence>
            </tbody>
