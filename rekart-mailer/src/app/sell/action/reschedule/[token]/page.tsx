...
import { publicActionService, type RescheduleInfo } from "@/services/public-action.service";
import {
  ActionShell,
  InfoRow,
  LoadingCard,
...
export default function ReschedulePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSub(true);
    try {
      await publicActionService.rescheduleSubmit(token, { preferredDate: date, preferredTime: time, note });
      setSent(true);
    } catch (e: unknown) {
