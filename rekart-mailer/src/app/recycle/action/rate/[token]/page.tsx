...
import { recyclePublicActionService } from "@/services/public-action.service";
import {
  ActionShell,
  LoadingCard,
  ErrorCard,
...
export default function RepairRatePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSub(true);
    try {
      await recyclePublicActionService.rateSubmit(token, { rating, feedback });
      setFinal(rating);
      setSent(true);
    } catch (e: unknown) {
    ...
    return (
      <ActionShell icon="⭐" title="Thank You!" subtitle={`Request ${requestId}`}>
        <div className="text-center py-4">
          <div className="text-5xl mb-3">{"⭐".repeat(finalRating)}</div>
          ...
          <p className="text-slate-500 text-sm">
            Your rating has been recorded. We&apos;ll use it to improve our repair service.
          </p>
        </div>
      </ActionShell>
    );
  ...
  return (
    <ActionShell icon="⭐" title="Rate Your Repair Experience" subtitle={`Request ${requestId}`}>
      <p className="text-sm text-slate-600 mb-5">
        Hi <strong>{customerName}</strong>, how was your repair experience with us?
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex justify-center gap-2 py-2">
          {[1, 2, 3, 4, 5].map((star) => (
            ...
            >
              <span className={(hover || rating) >= star ? "text-amber-400" : "text-slate-200"}>★</span>
            </button>
          ))}
        </div>
