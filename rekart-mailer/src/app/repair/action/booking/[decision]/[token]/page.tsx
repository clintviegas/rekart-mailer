...
import { repairPublicActionService, type BookingAckData } from "@/services/public-action.service";
import {
  ActionShell,
  InfoRow,
  LoadingCard,
...
export default function RepairBookingAckPage({
  params,
}: {
  ...
  useEffect(() => {
    const fn = isAccept
      ? repairPublicActionService.bookingAccept
      : repairPublicActionService.bookingDecline;
    fn(token)
      .then((r) => {
        setData(r.data);
  ...
  async function handleAcceptSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const r = await repairPublicActionService.bookingAcceptSubmit(token, {
