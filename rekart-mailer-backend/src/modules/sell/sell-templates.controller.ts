...
@Controller('sell')
export class SellTemplatesController {
  constructor(
    private readonly service: SellTemplatesService,
    private readonly testEmailService: SellTestEmailService,
    private readonly queueService: SellEmailQueueService,
  ) {}
  ...
  @Post('templates')
  @Roles(...WRITER_ROLES)
  create(@Body() dto: CreateSellTemplateDto, @CurrentUser() user: JwtPayload) {
  ...
  update(
    @Param('id') id: string,
    @Body() dto: UpdateSellTemplateDto,
    @CurrentUser() user: JwtPayload,
  ) {
