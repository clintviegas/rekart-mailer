...
import { JourneyActionType } from './schemas/sell-journey-action.schema';
...
export interface ActionTokenPayload {
  ...
  iat: number;
  /** offer-ready email revision; must match journey.offerGeneration when customer acts */
  offerGen?: number;
  /** request-received email revision; must match journey.requestAckGeneration when customer acts */
 ...
 */
@Injectable()
export class SellPublicActionTokenService {
  ...
  constructor() {
    ...
    if (!secret) {
      ...
      console.warn(
        '[SellPublicActionTokenService] WARNING: ACTION_TOKEN_SECRET is not set. ' +
  ...
  buildUrl(frontendUrl: string, actionPath: string, payload: ActionTokenPayload): string {
    ...
    return `${frontendUrl}/sell/action/${actionPath}/${token}`;
  }
}
