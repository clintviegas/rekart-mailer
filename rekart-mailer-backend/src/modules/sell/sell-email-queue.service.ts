...
@Injectable()
export class SellEmailQueueService {
  private readonly logger = new Logger(SellEmailQueueService.name);
  ...
  private requireQueue(): Queue {
    if (!this.queue) {
      throw new ServiceUnavailableException(
        'Email queue is off. Set SELL_USE_BULL_QUEUE=true and Redis, or send via SELL journeys (direct SMTP).',
      );
    }
    return this.queue;
  }
  ...
  async enqueue(dto: SendEmailDto, user: JwtPayload) {
    ...
    const job = await this.requireQueue().add(SELL_EMAIL_JOB, payload, {
  ...
  async resendOne(id: string, user: JwtPayload) {
    ...
    await this.requireQueue().add(SELL_EMAIL_JOB, payload, {
  ...
  async exportCsv(user: JwtPayload, filters: Omit<DeliveryLogFilters, 'page' | 'limit' | 'sortBy' | 'sortOrder'>) {
    ...
    const filename = `sell-delivery-logs-${new Date().toISOString().slice(0, 10)}.csv`;
