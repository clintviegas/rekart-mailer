...
@Controller('recycle/journeys')
export class RecycleJourneysController {
  constructor(
    private readonly service: RecycleJourneysService,
    private readonly actionService: RecyclePublicActionService,
  ) {}

  // POST /recycle/journeys
  @Post()
  @Roles(...WRITER_ROLES)
  ...
  findAll(
    ...
    @Query('revisedQuotes') revisedQuotes?: string,
  ...
  // GET /recycle/journeys/export — download all journeys as CSV (before :id)
  @Get('export')
  async exportCsv(
    @CurrentUser() user: JwtPayload,
    @Res() res: Response,
  ...
  // GET /recycle/journeys/stats — must come before :id
  @Get('stats')
  getStats(@CurrentUser() user: JwtPayload) {
    return this.service.getStats(user);
  }

  // GET /recycle/journeys/analytics — full analytics data (trends, funnel, actions)
  @Get('analytics')
  getAnalytics(@CurrentUser() user: JwtPayload) {
    return this.service.getJourneyAnalytics(user);
  }

  // GET /recycle/journeys/staff-notifications — bell + unread count (before :id)
  @Get('staff-notifications')
  getStaffNotifications(@CurrentUser() user: JwtPayload) {
    return this.service.getStaffNotifications(user);
  }

  // POST /recycle/journeys/staff-notifications/mark-read
  ...
  // GET /recycle/journeys/by-request/:requestId — must come before :id
  ...
  // GET /recycle/journeys/:id/quote-email-logs — every quote-ready send (for per-round preview)
  @Get(':id/quote-email-logs')
  listOfferEmailLogs(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
    return this.service.listQuoteEmailDeliveryLogs(id, user);
  }

  // GET /recycle/journeys/:id/delivery-log/:logId/preview — HTML exactly as stored on send
  ...
  // GET /recycle/journeys/:id
  @Get(':id')
  findOne(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
    return this.service.findOne(id, user);
  }

  // POST /recycle/journeys/:id/send-next
  @Post(':id/send-next')
  @Roles(...SENDER_ROLES)
  sendNext(
    @Param('id') id: string,
  ...
  // POST /recycle/journeys/:id/send-step/:step
  ...
  // POST /recycle/journeys/:id/resend/:step
  @Post(':id/resend/:step')
  @Roles(...SENDER_ROLES)
  resendStep(
    @Param('id') id: string,
  ...
  // POST /recycle/journeys/:id/preview/:step
  ...
  // GET /recycle/journeys/:id/actions — customer action log for dashboard
  @Get(':id/actions')
  getActions(
    @Param('id') id: string,
    @CurrentUser() user: JwtPayload,
  ...
  // PATCH /recycle/journeys/:id/cancel
  @Patch(':id/cancel')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  ...
  // PATCH /recycle/journeys/:id/decline-quote
  @Patch(':id/decline-quote')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  declineOffer(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
    return this.service.markQuoteDeclined(id, user);
  }

  // PATCH /recycle/journeys/:id/data — update dynamic data fields (whitelisted keys only)
  @Patch(':id/data')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  updateData(
  ...
  // POST /recycle/journeys/:id/attachments — add an attachment ref to a journey
  @Post(':id/attachments')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  addAttachment(
  ...
  // DELETE /recycle/journeys/:id/attachments/:filename — remove an attachment ref
  ...
  // POST /recycle/journeys/:id/notes — add a staff internal note
  @Post(':id/notes')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  addNote(
  ...
  // DELETE /recycle/journeys/:id/notes/:index — remove a staff note by index
