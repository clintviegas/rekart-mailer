...
import { EXTRA_RECYCLE_WORKFLOW_KEYS } from './recycle-template-defaults';
import { ADHOC_RETURN_RECYCLE_WORKFLOW_KEYS } from './recycle-template-defaults';
import { COURIER_DISPATCH_RECYCLE_WORKFLOW_KEYS } from './recycle-template-defaults';
...
} from './templates/recycle-email-html.renderer';
import {
  RecyclePublicActionTokenService,
  type ActionTokenPayload,
} from './recycle-public-action-token.service';
import {
  JourneyActionType,
...
import {
  ...
  RecycleStaffNotificationStateDocument,
...
const Recycle_ATTACHMENTS_DIR = path.join(process.cwd(), 'uploads', 'recycle-attachments');
...
function interpolateRecycleTemplateVars(
  text: string,
...
@Injectable()
export class RecycleJourneysService implements OnModuleInit {
  private readonly logger = new Logger(RecycleJourneysService.name);

  onModuleInit() {
    const mode =
      process.env.RECYCLE_USE_BULL_QUEUE === 'true'
        ? 'BullMQ (Redis queue) — enabled'
        : 'Direct SMTP — no Bull queue/Redis for journeys';
    this.logger.log(`Recycle journey step emails: ${mode}`);
  }
  ...
  constructor(
    ...
    private readonly journeyActionModel: Model<RecycleJourneyActionDocument>,

    @InjectModel(RecycleStaffNotificationState.name)
    private readonly staffNotifStateModel: Model<RecycleStaffNotificationStateDocument>,
    ...
    private readonly actionTokenService: RecyclePublicActionTokenService,
    private readonly RecycleTemplatesService: RecycleTemplatesService,
  ) {}
  ...
  async getStats(user: JwtPayload) {
    ...
    const [statusAgg, stepAgg, currentStepAgg, revenueAgg, recentDocs, recentActions, revisedQuoteJourneys, revisedOfferSendsSum, reshipClosed, reminderDueCount] =
      await Promise.all([
        ...
        this.journeyModel.aggregate([
          { $match: { workspaceId: wid, status: RecycleJourneyStatus.ACTIVE } },
          { $group: { _id: '$currentStep', count: { $sum: 1 } } },
        ]),

        // Revenue from paidAmount in dynamicData
        this.journeyModel.aggregate([
          { $match: { workspaceId: wid, status: RecycleJourneyStatus.COMPLETED } },
          {
            $group: {
              _id: null,
              total: { $sum: { $literal: 0 } },
    ...
    const paymentsIssued = byStep['recycle-in-progress']   ?? 0;
  ...
  async sendReminderEmail(
    journey: RecycleRequestJourneyDocument,
    reminderType:
      | 'recycle-request'
      | 'pickup-scheduled'
      | 'quote-ready'
    ...
    const mergedData: Record<string, unknown> = {
      ...
      reminderTotal:  reminderType === 'recycle-request' ? '1' : '3',
    };
    ...
    if (reminderType === 'recycle-request') {
      const gen = journey.requestAckGeneration ?? 1;
      this.injectActionUrls(ctx, 'recycle-request', mergedData, { bookingAckGen: gen });
    ...
    try {
      ...
      const tmpl = await this.RecycleTemplatesService.ensurePublishedTemplate(reminderKey, fakeUser);
      const rawSubject = tmpl.subject ?? subject;
      subject = interpolateRecycleTemplateVars(rawSubject, mergedData);
      templateId = (tmpl._id as Types.ObjectId) ?? null;
    } catch {
      // Template doesn't exist yet — use fallback subject
      const fallbacks: Record<string, string> = {
        'recycle-request': `Reminder: Please confirm your e-waste pickup — #${journey.requestId}`,
        'pickup-scheduled': `Reminder: Your recycling pickup is coming up — #${journey.requestId}`,
      };
  ...
  async getJourneyAnalytics(user: JwtPayload) {
    ...
    const [
      ...
      this.journeyModel.aggregate([
        ...
        { $group: {
          ...
          requests:      { $sum: 1 },
          completions:   { $sum: { $cond: [{ $eq: ['$status', RecycleJourneyStatus.COMPLETED] }, 1, 0] } },
          revenue: { $sum: { $literal: 0 } },
          cancellations: { $sum: { $cond: [{ $in: ['$status', [RecycleJourneyStatus.CANCELLED, RecycleJourneyStatus.QUOTE_DECLINED, RecycleJourneyStatus.REQUEST_DECLINED]] }, 1, 0] } },
        }},
      ...
      this.journeyModel.aggregate([
        ...
        {
          $group: {
            _id: { $ifNull: ['$dynamicData.collectionMode', 'listed'] },
            count: { $sum: 1 },
          },
        },
      ]),
    ...
    const FUNNEL_STEPS = [...RECYCLE_JOURNEY_WORKFLOW_STEPS] as const;
    ...
    const collectionModeBreakdown = collectionModeRaw.map((r) => ({
      mode: String(r._id ?? 'listed'),
      label: String(r._id ?? 'listed') === 'bulk_estimate' ? 'Bulk / office clearance' : 'Known item list',
      count: r.count as number,
    }));

    const byStatus: Record<string, number> = {};
    for (const row of statusRaw) byStatus[row._id as string] = row.count as number;
  ...
  async sendNext(id: string, dto: SendJourneyStepDto, user: JwtPayload) {
    ...
    if (journey.status === RecycleJourneyStatus.COMPLETED) {
  ...
  async sendStep(
    ...
    this.validateStepKey(step);
    const isAdhocReturn = (ADHOC_RETURN_RECYCLE_WORKFLOW_KEYS as readonly string[]).includes(step);
    const isCourierDispatch = (COURIER_DISPATCH_RECYCLE_WORKFLOW_KEYS as readonly string[]).includes(step);
    if ((EXTRA_RECYCLE_WORKFLOW_KEYS as readonly string[]).includes(step)) {
    ...
    const stepIndex = RECYCLE_JOURNEY_WORKFLOW_STEPS.indexOf(step as any);
    ...
    if (stepIndex > nextIndex) {
      throw new BadRequestException(
        `Cannot send "${step}" before completing "${RECYCLE_JOURNEY_WORKFLOW_STEPS[nextIndex]}".`,
      );
    }
  ...
  async previewStep(
    ...
    this.validateStepKey(step);
    const stepIndex = RECYCLE_JOURNEY_WORKFLOW_STEPS.indexOf(step as (typeof RECYCLE_JOURNEY_WORKFLOW_STEPS)[number]);
    const alreadySent = journey.sentSteps.some((s) => s.stepKey === step);
    ...
    if (!alreadySent) {
      ...
      if (stepIndex > nextIndex) {
        throw new BadRequestException(
          `Cannot preview "${step}" before completing "${RECYCLE_JOURNEY_WORKFLOW_STEPS[nextIndex]}".`,
        );
      }
    }
   ...
   * Uses the same `renderRecycleEmailHtml` pipeline as journey step preview.
   */
  ...
  private assertCustomerAcknowledgedRequest(journey: {
    ...
    if (!journey.completedSteps?.includes('recycle-request')) {
      return;
    }
    const gen =
  ...
  private assertCourierDispatchAllowed(journey: {
    ...
    if (journey.currentStep !== 'certificate-issued') {
      throw new BadRequestException(
  ...
  private async dispatchStep(
    ...
    if (stepKey === 'recycle-in-progress') {
    ...
    const template = await this.RecycleTemplatesService.ensurePublishedTemplate(
      stepKey,
      user,
    );
    ...
    const subject = interpolateRecycleTemplateVars(subjectRaw, mergedData);
    ...
    const useBullQueue = process.env.RECYCLE_USE_BULL_QUEUE === 'true';
    ...
    if (!useBullQueue) {
      ...
      if (!this.queue) {
        throw new ServiceUnavailableException(
          'Bull queue is not enabled. Set RECYCLE_USE_BULL_QUEUE=true, start Redis, then restart the server.',
        );
      }
      try {
        await this.queue.add(RECYCLE_EMAIL_JOB, payload, {
    ...
    if (opts?.adhocReturn) {
      ...
      if (stepKey === 'device-return-unrecycled-complete') {
        adhocSet['status'] = RecycleJourneyStatus.COMPLETED;
      ...
      await this.journeyModel.updateOne(
        { _id: journey._id },
        {
          ...
          $set: {
            ...dynPatch,
            ...(stepKey === 'device-return-unrecycled-complete'
              ? { status: RecycleJourneyStatus.COMPLETED, 'dynamicData.closedByDeviceReturn': true }
              : {}),
          },
        },
      );
      ...
      if (stepKey === 'recycle-request' && bookingAckGenForTokens != null) {
        Object.assign(resendSet, {
          requestAckGeneration: bookingAckGenForTokens,
          status: RecycleJourneyStatus.ACTIVE,
          currentStep: 'recycle-request',
      ...
      const newStatus =
        newCompleted.length === RECYCLE_JOURNEY_WORKFLOW_STEPS.length
          ? RecycleJourneyStatus.COMPLETED
          : RecycleJourneyStatus.ACTIVE;
      ...
      const firstSendRequestAckPatch =
        stepKey === 'recycle-request' && bookingAckGenForTokens != null
          ? { requestAckGeneration: bookingAckGenForTokens }
          : {};
  ...
  private async renderJourneyStepHtmlForContext(
    ...
    this.injectActionUrls(
      ...
      stepKey === 'quote-ready'
        ...
        : stepKey === 'recycle-request'
          ? { bookingAckGen: this.nextrequestAckGenerationNumber(ctx) }
          : undefined,
    );
    ...
    const html = renderRecycleEmailHtml(stepKey, mergedData, {
      workspaceName,
      logoUrl,
      mascotSrc,
      socialIconSrcs,
  ...
  private async sendDirectFallback(
    payload: RecycleEmailJobPayload,
    logId: string,
  ): Promise<void> {
    try {
      const [workspace, branding] = await Promise.all([
      ...
      const html = renderRecycleEmailHtml(
        payload.workflowKey,
      ...
      const emailAttachments = (payload.attachments ?? []).map((a) => ({
        ...
        storedPath: path.join(Recycle_ATTACHMENTS_DIR, a.storedFilename),
  ...
  async cancel(id: string, user: JwtPayload) {
    ...
    if (journey!.status === RecycleJourneyStatus.CANCELLED) {
      throw new BadRequestException('Journey is already cancelled');
    }

    await this.journeyModel.updateOne(
      { _id: new Types.ObjectId(id) },
      { $set: { status: RecycleJourneyStatus.CANCELLED } },
    );

    this.logger.log(`Journey ${journey!.requestId} cancelled by ${user.sub}`);
    return createResponse({ id, status: RecycleJourneyStatus.CANCELLED }, 'Journey cancelled');
  }
  ...
  async markQuoteDeclined(id: string, user: JwtPayload) {
    ...
    if ([RecycleJourneyStatus.CANCELLED, RecycleJourneyStatus.COMPLETED].includes(journey!.status as RecycleJourneyStatus)) {
    ...
    return createResponse({ id, status: RecycleJourneyStatus.QUOTE_DECLINED }, 'Offer marked as declined');
  }
  ...
  async previewDeliveryLog(
    ...
    const html = renderRecycleEmailHtml(log.workflowKey, log.dynamicFieldValues ?? {}, {
      workspaceName,
      logoUrl,
      mascotSrc,
      socialIconSrcs,
  ...
  private toPreviewContext(doc: RecycleRequestJourneyDocument): JourneyPreviewContext {
    return {
  ...
  private injectActionUrls(
    ...
    switch (stepKey) {
      case 'recycle-request': {
        const gen =
          opts?.bookingAckGen ?? this.nextrequestAckGenerationNumber(ctx);
      ...
      case 'devices-collected':
      case 'diagnosing':
      case 'recycle-in-progress':
        data['trackUrl'] = buildUrl('track', JourneyActionType.TRACK_CLICKED);
        break;
  ...
  private getNextStep(completedSteps: string[]): string | null {
    const step = RECYCLE_JOURNEY_WORKFLOW_STEPS.find((s) => !completedSteps.includes(s));
    return step ?? null;
  }

  private getNextStepIndex(completedSteps: string[]): number {
    const next = this.getNextStep(completedSteps);
    if (!next) return RECYCLE_JOURNEY_WORKFLOW_STEPS.length;
    return RECYCLE_JOURNEY_WORKFLOW_STEPS.indexOf(next as any);
  }
