...
import {
  RecycleSuppression,
  RecycleSuppressionDocument,
  SuppressionReason,
} from './schemas/recycle-suppression.schema';
...
@Injectable()
export class RecycleSuppressionService {
  constructor(
    @InjectModel(RecycleSuppression.name)
    private readonly model: Model<RecycleSuppressionDocument>,
  ) {}
    ...
    try {
      ...
      if (e.code === 11000) {
        ...
        const existing = await this.model
          ...
          .lean();
        return existing as RecycleSuppression;
      }
      throw err;
    }
  }
