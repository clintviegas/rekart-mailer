...
import { JourneyActionType } from './schemas/recycle-journey-action.schema';
...
export interface ActionTokenPayload {
  ...
  quoteGen?: number;
  /** booking-confirmed email revision; must match journey.requestAckGeneration when customer acts */
 ...
 */
@Injectable()
export class RecyclePublicActionTokenService {
  ...
  constructor() {
    ...
    if (!secret) {
      ...
      console.warn(
        '[RecyclePublicActionTokenService] WARNING: ACTION_TOKEN_SECRET is not set. ' +
  ...
  buildUrl(frontendUrl: string, actionPath: string, payload: ActionTokenPayload): string {
    ...
    return `${frontendUrl}/recycle/action/${actionPath}/${token}`;
  }
}
