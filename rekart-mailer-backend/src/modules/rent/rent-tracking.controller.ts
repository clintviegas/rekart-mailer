...
@Controller('rent/track')
export class RentTrackingController {
  private readonly logger = new Logger(RentTrackingController.name);
  ...
  async trackClick(
    ...
    if (payload) {
      const now = new Date();
      const safeUrl = destinationUrl.slice(0, 2048);
      try {
        await this.logModel.findOneAndUpdate(
          { _id: payload.lid, trackingToken: token },
          [
