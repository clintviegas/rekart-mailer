...
export function computeRentItemChanges(
  ...
  for (const item of original) {
    ...
    if (!key) continue;
    origMap.set(key, { name: item.name.trim(), qty: Math.max(1, item.qty) });
  }
  ...
  for (const item of confirmed) {
    const key = norm(item.name);
    if (!key) continue;
    confMap.set(key, { name: item.name.trim(), qty: Math.max(1, item.qty) });
  }
  ...
  for (const [key, c] of confMap) {
    ...
    if (!o) added.push(c);
      ...
      qtyChanged.push({ name: c.name, from: o.qty, to: c.qty });
    }
  }
