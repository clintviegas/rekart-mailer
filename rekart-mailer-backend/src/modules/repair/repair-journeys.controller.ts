...
@Controller('repair/journeys')
export class RepairJourneysController {
  constructor(
    private readonly service: RepairJourneysService,
    private readonly actionService: RepairPublicActionService,
  ) {}

  // POST /repair/journeys
  @Post()
  @Roles(...WRITER_ROLES)
  ...
  // GET /repair/journeys/export — download all journeys as CSV (before :id)
  @Get('export')
  async exportCsv(
    @CurrentUser() user: JwtPayload,
    @Res() res: Response,
  ...
  // GET /repair/journeys/stats — must come before :id
  @Get('stats')
  getStats(@CurrentUser() user: JwtPayload) {
    return this.service.getStats(user);
  }

  // GET /repair/journeys/analytics — full analytics data (trends, funnel, actions)
  @Get('analytics')
  getAnalytics(@CurrentUser() user: JwtPayload) {
    return this.service.getJourneyAnalytics(user);
  }

  // GET /repair/journeys/staff-notifications — bell + unread count (before :id)
  @Get('staff-notifications')
  getStaffNotifications(@CurrentUser() user: JwtPayload) {
    return this.service.getStaffNotifications(user);
  }

  // POST /repair/journeys/staff-notifications/mark-read
  ...
  // GET /repair/journeys/by-request/:requestId — must come before :id
  ...
  // GET /repair/journeys/:id/quote-email-logs — every quote-ready send (for per-round preview)
  ...
  // GET /repair/journeys/:id/delivery-log/:logId/preview — HTML exactly as stored on send
  ...
  // GET /repair/journeys/:id
  @Get(':id')
  findOne(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
    return this.service.findOne(id, user);
  }

  // POST /repair/journeys/:id/send-next
  @Post(':id/send-next')
  @Roles(...SENDER_ROLES)
  sendNext(
    @Param('id') id: string,
  ...
  // POST /repair/journeys/:id/send-step/:step
  ...
  // POST /repair/journeys/:id/resend/:step
  @Post(':id/resend/:step')
  @Roles(...SENDER_ROLES)
  resendStep(
    @Param('id') id: string,
  ...
  // POST /repair/journeys/:id/preview/:step
  ...
  // GET /repair/journeys/:id/actions — customer action log for dashboard
  @Get(':id/actions')
  getActions(
    @Param('id') id: string,
    @CurrentUser() user: JwtPayload,
  ...
  // PATCH /repair/journeys/:id/cancel
  @Patch(':id/cancel')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  ...
  // PATCH /repair/journeys/:id/decline-quote
  ...
  // PATCH /repair/journeys/:id/data — update dynamic data fields (whitelisted keys only)
  @Patch(':id/data')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  updateData(
  ...
  // POST /repair/journeys/:id/attachments — add an attachment ref to a journey
  @Post(':id/attachments')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  addAttachment(
  ...
  // DELETE /repair/journeys/:id/attachments/:filename — remove an attachment ref
  ...
  // POST /repair/journeys/:id/notes — add a staff internal note
  @Post(':id/notes')
  @Roles(...WRITER_ROLES)
  @HttpCode(HttpStatus.OK)
  addNote(
  ...
  // DELETE /repair/journeys/:id/notes/:index — remove a staff note by index
