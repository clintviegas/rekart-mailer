...
@Injectable()
export class RepairEmailQueueService {
  private readonly logger = new Logger(RepairEmailQueueService.name);
  ...
  private requireQueue(): Queue {
    if (!this.queue) {
      throw new ServiceUnavailableException(
        'Email queue is off. Set REPAIR_USE_BULL_QUEUE=true and Redis, or send via SELL journeys (direct SMTP).',
      );
    }
    return this.queue;
  }
  ...
  async enqueue(dto: SendEmailDto, user: JwtPayload) {
    ...
    const job = await this.requireQueue().add(REPAIR_EMAIL_JOB, payload, {
  ...
  async resendOne(id: string, user: JwtPayload) {
    ...
    await this.requireQueue().add(REPAIR_EMAIL_JOB, payload, {
      ...
      logs.map(async (log) => {
        ...
        await this.requireQueue().add(
          REPAIR_EMAIL_JOB,
          {
            ...
            triggeredBy: user.sub,
            deliveryLogId: (newLog._id as Types.ObjectId).toString(),
          } satisfies RepairEmailJobPayload,
  ...
  async exportCsv(user: JwtPayload, filters: Omit<DeliveryLogFilters, 'page' | 'limit' | 'sortBy' | 'sortOrder'>) {
    ...
    const filename = `repair-delivery-logs-${new Date().toISOString().slice(0, 10)}.csv`;
