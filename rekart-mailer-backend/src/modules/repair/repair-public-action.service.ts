...
import {
  ...
  RepairJourneyWorkflowStep,
} from './schemas/repair-request-journey.schema';
import { RepairPublicActionTokenService, ActionTokenPayload } from './repair-public-action-token.service';
import { createResponse } from '../../common/utils/api-response';
import { isValidCustomerPreferredPickupDate, normalizePickUpAddress } from '../../common/date-format';
import { isValidRepairReturnPaymentMethod } from './repair-payment-methods';
import { isValidQuoteDeclineReason } from './repair-quote-decline-reasons';
import {
  REPAIR_PICKUP_TIME_WINDOW_SELECT_OPTIONS,
  isRepairPickupTimeAny,
  isValidRepairPickupTimeWindowSelection,
} from './repair-pickup-time-windows';
...
@Injectable()
export class RepairPublicActionService {
  private readonly logger = new Logger(RepairPublicActionService.name);
  ...
  constructor(
    ...
    private readonly tokenService: RepairPublicActionTokenService,
  ) {}
  ...
  private async resolveJourney(jid: string) {
    const journey = await this.journeyModel
      ...
      .select(
        'requestId customerName customerEmail currency currentStep status completedSteps workspaceId sentSteps dynamicData quoteGeneration bookingAckGeneration',
      )
      .lean();
  ...
  private async logAction(
    ...
    ctx: ActionContext,
  ): Promise<RepairJourneyActionDocument> {
  ...
  async handleOfferAcceptView(token: string, ctx: ActionContext) {
    ...
    if (
      journey.status === RepairJourneyStatus.CANCELLED ||
      journey.status === RepairJourneyStatus.COMPLETED
    ) {
      const note =
        journey.status === RepairJourneyStatus.COMPLETED
          ? 'This repair request is already complete. Thank you for choosing Rekart!'
    ...
    if (journey.status === RepairJourneyStatus.QUOTE_DECLINED) {
      return createResponse(
        this.buildQuoteActionPayload(journey, {
          decision: 'accept',
          alreadyProcessed: true,
    ...
    if (Number(d['quoteAcceptedGeneration'] ?? 0) === jGen && d['quoteAcceptedByCustomer'] === true) {
      return createResponse(
        ...
        'Your quote acceptance is already recorded. Our team will proceed with the repair.',
      );
    }
  ...
  async handleOfferAcceptSubmit(
    ...
    if (!isValidRepairReturnPaymentMethod(preferredPaymentMethod)) {
    ...
    if (
      ...
      return createResponse(
        ...
        journey.status === RepairJourneyStatus.COMPLETED
          ? 'This repair request is already complete.'
    ...
    if (
      journey.status === RepairJourneyStatus.DEVICE_RETURN_REQUESTED ||
    ...
    return createResponse(
      ...
      'Quote accepted! We will start the repair and collect payment when your device is returned.',
    );
  }
  ...
  async handleOfferDeclineView(token: string, ctx: ActionContext) {
    ...
    if (
      journey.status === RepairJourneyStatus.QUOTE_DECLINED &&
  ...
  async handleReturnDeviceView(token: string, ctx: ActionContext) {
    ...
    if (journey.status === RepairJourneyStatus.DEVICE_RETURN_REQUESTED || d['returnDeviceRequested'] === true) {
      return createResponse(
  ...
  async handleReturnDeviceSubmit(token: string, ctx: ActionContext) {
    ...
    const result = await this.journeyModel.updateOne(
      ...
      {
        $set: {
          status: RepairJourneyStatus.DEVICE_RETURN_REQUESTED,
          currentStep: 'quote-ready',
  ...
  private buildBookingAckPayload(
    ...
    return {
      ...
      timeWindowOptions: [...REPAIR_PICKUP_TIME_WINDOW_SELECT_OPTIONS],
    };
  }
  ...
  async handleBookingAcceptView(token: string, ctx: ActionContext) {
    ...
    const reqIdx = REPAIR_JOURNEY_WORKFLOW_STEPS.indexOf('booking-confirmed');
    const progressedPastRequest = (journey.completedSteps ?? []).some((s: string) => {
      const i = REPAIR_JOURNEY_WORKFLOW_STEPS.indexOf(s as RepairJourneyWorkflowStep);
      return i !== -1 && i > reqIdx;
    });
    ...
    const ackGen = dd['bookingAckAcceptedGen'];
    const acceptedGen =
      typeof ackGen === 'number' && ackGen > 0 ? ackGen : null;
    const alreadyAccepted =
      dd['bookingAckByCustomer'] === true &&
      acceptedGen !== null &&
      acceptedGen === jGen;
  ...
  async handleBookingAcceptSubmit(
    ...
    if (!isValidRepairPickupTimeWindowSelection(slot)) {
    ...
    const { p, journey, jGen, tGen } = await this.validateBookingAckToken(token, 'accept');
    const dd = (journey.dynamicData ?? {}) as Record<string, unknown>;
    const anyTime = isRepairPickupTimeAny(slot);
    ...
    const ackAtomicFilter = {
      _id: new Types.ObjectId(p.jid),
      bookingAckGeneration: jGen,
      'dynamicData.bookingAckAcceptedGen': { $ne: jGen },
    };

    const result = await this.journeyModel.updateOne(ackAtomicFilter, {
      $set: {
        status: RepairJourneyStatus.ACTIVE,
        'dynamicData.bookingAckByCustomer': true,
        'dynamicData.bookingAckAcceptedGen': jGen,
    ...
    await this.logAction(
      p,
      JourneyActionType.BOOKING_CONFIRMED_ACCEPTED,
      { decision: 'accept', bookingAckGeneration: jGen, preferredPickupTimeSlot: slot, preferredPickupDate: date, anyTime },
      ctx,
    );
  ...
  async handleRequestReceivedDecision(
    ...
    if (jGen <= 0) {
      await this.logAction(
        p,
        JourneyActionType.BOOKING_CONFIRMED_DECLINED,
        { decision, bookingAckGeneration: jGen, legacy: true },
        ctx,
      );
      const r = baseResponse(true);
      return {
    ...
    const decGen = dd['bookingAckDeclinedGen'];
    const declinedGen =
      typeof decGen === 'number' && decGen > 0 ? decGen : null;
    const alreadyDeclinedForRound =
      journey.status === RepairJourneyStatus.BOOKING_DECLINED &&
      dd['bookingAckDeclined'] === true &&
      declinedGen !== null &&
      declinedGen === jGen;
    ...
    if (alreadyDeclinedForRound) {
      await this.logAction(
        p,
        JourneyActionType.BOOKING_CONFIRMED_DECLINED,
        { decision, bookingAckGeneration: jGen, duplicate: true },
        ctx,
      );
      const r = baseResponse(true);
      return {
    ...
    const result = await this.journeyModel.updateOne(ackAtomicFilter, {
      $set: {
        status: RepairJourneyStatus.BOOKING_DECLINED,
        currentStep: 'booking-confirmed',
        'dynamicData.bookingAckByCustomer': false,
        'dynamicData.bookingAckDeclined': true,
        'dynamicData.bookingAckDeclinedGen': jGen,
        'dynamicData.bookingAckDeclinedAt': new Date(),
        'dynamicData.reminderDue': false,
      },
      $unset: {
    ...
    await this.logAction(
      p,
      JourneyActionType.BOOKING_CONFIRMED_DECLINED,
      { decision, bookingAckGeneration: jGen },
      ctx,
    );
