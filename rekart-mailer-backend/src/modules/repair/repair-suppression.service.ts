...
import {
  RepairSuppression,
  RepairSuppressionDocument,
  SuppressionReason,
} from './schemas/repair-suppression.schema';
...
@Injectable()
export class RepairSuppressionService {
  constructor(
    @InjectModel(RepairSuppression.name)
    private readonly model: Model<RepairSuppressionDocument>,
  ) {}
    ...
    try {
      ...
      if (e.code === 11000) {
        ...
        const existing = await this.model
          ...
          .lean();
        return existing as RepairSuppression;
      }
      throw err;
    }
  }
