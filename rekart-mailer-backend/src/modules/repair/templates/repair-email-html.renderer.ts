...
const HERO_HEADLINES: Record<string, string> = {
  'booking-confirmed':          'Your repair booking is confirmed!',
  'booking-confirmed-reminder': 'Still waiting for your confirmation',
  'pickup-scheduled':          'Your pickup is confirmed!',
  'pickup-scheduled-reminder': 'Reminder: pickup coming up',
  'device-received':       'Diagnosis in progress',
  'quote-ready':               'Your repair quote is ready!',
  'quote-ready-reminder':      'Your quote is still waiting',
  'repair-in-progress':              'Repair work has started!',
  'device-ready':                    'Your device is ready for collection!',
  'device-ready-reminder':           'Reminder: your device is ready',
  'device-returned':                 'Repair complete — thank you!',
  'courier-dispatched':              'Your repaired device is on the way!',
};

const REPAIR_PROGRESS_STEPS = [
  { key: 'booking-confirmed', label: 'Booked' },
  { key: 'pickup-scheduled', label: 'Pickup' },
  { key: 'device-received', label: 'Received' },
  { key: 'diagnosing', label: 'Diagnose' },
  { key: 'quote-ready', label: 'Quote' },
  { key: 'repair-in-progress', label: 'Repair' },
  { key: 'device-ready', label: 'Ready' },
  { key: 'device-returned', label: 'Returned' },
];

const REPAIR_DISPATCH_PROGRESS_STEP = { key: 'courier-dispatched', label: 'Dispatch' };

function readRepairReturnModeForProgress(f?: FieldData): 'store' | 'courier' | null {
  const raw = String(f?.returnMode ?? '').trim().toLowerCase();
  if (raw === 'store') return 'store';
  if (raw === 'courier') return 'courier';
  return null;
}

/** Show Dispatch column on device-ready stage and later (store = skipped, courier = tracked). */
function progressStepsForEmail(workflowKey: string): { key: string; label: string }[] {
  const base = workflowKey.replace(/-reminder$/, '');
  const includeDispatch = [
    'device-ready',
    'device-ready-reminder',
    'courier-dispatched',
    'device-returned',
  ].includes(base);
  if (!includeDispatch) return REPAIR_PROGRESS_STEPS;
  const steps = [...REPAIR_PROGRESS_STEPS];
  const returnedIdx = steps.findIndex((s) => s.key === 'device-returned');
  steps.splice(returnedIdx, 0, REPAIR_DISPATCH_PROGRESS_STEP);
  return steps;
}

function progressStepIndex(workflowKey: string, steps: { key: string; label: string }[]): number {
  const base = workflowKey.replace(/-reminder$/, '');
  if (base === 'device-ready-reminder') {
    return Math.max(0, steps.findIndex((s) => s.key === 'device-ready'));
  }
  if (base === 'device-return-unrepaired' || base === 'device-return-unrepaired-complete') {
    return steps.length - 1;
  }
  if (base === 'courier-dispatched') {
    const idx = steps.findIndex((s) => s.key === 'courier-dispatched');
...
/** Unrepaired return: Booked→Quote done, Repair/Ready/Dispatch skipped, Returned active. */
function unrepairedReturnProgressStates(
  workflowKey: string,
...
function repairProgressVisualStates(
  workflowKey: string,
  steps: { key: string; label: string }[],
  f?: FieldData,
): ProgressStepVisualState[] {
  const unrepairedStates = unrepairedReturnProgressStates(workflowKey, steps);
  if (unrepairedStates) return unrepairedStates;

  const activeIdx = progressStepIndex(workflowKey, steps);
  const returnMode = readRepairReturnModeForProgress(f);
  const dispatchIdx = steps.findIndex((s) => s.key === 'courier-dispatched');

  return steps.map((step, i) => {
    if (returnMode === 'store' && step.key === 'courier-dispatched') {
      return 'skipped';
    }
    if (i === activeIdx) return 'current';
    if (i < activeIdx) {
      if (returnMode === 'store' && i === dispatchIdx) return 'skipped';
      return 'completed';
    }
    return 'pending';
  });
...
function stepProgressHtml(workflowKey: string, primary: string, f?: FieldData): string {
  ...
  const states = repairProgressVisualStates(workflowKey, steps, f);
  const cells = steps
...
const WORKFLOW_MASCOTS: Record<string, { file: string; alt: string }> = {
  'booking-confirmed':          { file: 'mascot-request.png',    alt: 'Rekart mascot with phone' },
  'booking-confirmed-reminder': { file: 'mascot-request.png',    alt: 'Rekart mascot with phone' },
  ...
  'device-received':       { file: 'mascot-inspection.png', alt: 'Rekart mascot on laptop' },
  'quote-ready':               { file: 'mascot-offer.png',      alt: 'Rekart mascot' },
  'quote-ready-reminder':      { file: 'mascot-offer.png',      alt: 'Rekart mascot' },
  'repair-in-progress':              { file: 'mascot-complete.png',   alt: 'Rekart mascot with recycle symbol' },
  completed:                   { file: 'mascot-complete.png',   alt: 'Rekart mascot celebrating' },
  'device-returned':             { file: 'mascot-reship.png',     alt: 'Rekart mascot' },
  'courier-dispatched':          { file: 'mascot-reship.png',     alt: 'Rekart mascot' },
};
...
const WORKFLOW_CONTENT: Record<string, WorkflowBuilderFn> = {
  'booking-confirmed': (f, cb, t, ov) => {
    ...
    const pickupScheduleHint = `<p style="margin:0 0 14px;font-size:12px;line-height:1.55;color:#475569;">To schedule a free home pickup, tap <strong style="color:${REKART_BRAND_LIGHT};">Schedule Pickup</strong> below. Our agent will collect your device for repair at our service centre.</p>`;
  ...
  'pickup-scheduled': (f, _cb, t) => {
    ...
    return [
      ...
      alertBox(
        'Please have your device and original accessories ready.',
        'success',
      ),
    ].filter(Boolean).join('');
  },
  ...
  'device-received': (f) => [
    deviceCard(f),
    infoTable('Intake Status', [
      ['Request ID', f.requestId],
      ['Received By', f.receivedBy || f.inspectorName],
      ['Received On', dv(f.receivedAt || f.inspectionStartTime || f.inspectionStartDate)],
    ]),
    alertBox('Your item is safely at our service centre. Our technicians will begin diagnosis shortly and share a repair quote once ready.', 'info'),
  ].join(''),
  ...
  'diagnosing': (f) => [
    ...
    alertBox('Our technicians are diagnosing your item. We will email you a repair quote as soon as the assessment is complete.', 'warning'),
  ].join(''),
  ...
  'device-ready': (f, cb, t) => {
    ...
    return [
      ...
      alertBox(
        'Your device repair is complete! Please choose how you would like to receive it using the buttons below.',
        'success',
      ),
  ...
  'quote-ready': (f, cb, t, ov) => {
    ...
    const repairSummary = String(f.repairSummary ?? '').trim();
    const summaryBlock = repairSummary
      ? alertBox(
          `<strong style="display:block;margin-bottom:6px;font-size:13px;color:#0f172a;">Work included / Diagnosis summary</strong><span style="font-size:14px;line-height:1.55;color:#334155;">${escapeHtmlEntities(repairSummary).replace(/\n/g, '<br/>')}</span>`,
          'info',
        )
      : '';
    ...
    if (reviseType === 'after_reason' && declineReason) {
      ...
      headerCallout = alertBox(
        `<strong style="font-size:14px;">Our best offer</strong><br/><span style="font-size:13px;line-height:1.55;">We cannot reduce the price further. You can accept this quote to proceed with repair, or request return of your device without repair.</span>`,
        'warning',
      );
    } else if (revN > 1) {
      headerCallout = alertBox(
    ...
    const paymentNote = reviseType !== 'final_offer'
      ? alertBox(
          'By accepting this quote, we will start the repair process. Payment is collected when your device is returned — please choose your preferred payment method on the accept page.',
          'info',
        )
      : alertBox(
          'If you accept, we will start the repair and collect payment when your device is returned. If you prefer not to proceed, you can request return of your device as-is.',
          'info',
        );
  ...
  'repair-in-progress': (f, cb, t) => {
    const estDate = dv(String(f.estimatedCompletionDate ?? '').trim());
    const notes = String(f.repairNotes ?? '').trim();
    ...
    return [
      highlight('Repair In Progress', '', 'Work has started on your device'),
      infoTable('Repair Status', detailRows),
  ...
  'device-returned': (f, cb, t) => {
    const finalAmt = f.finalAmount || f.paidAmount || f.paymentAmount;
    const currency = f.currency || '';
    const displayAmt = finalAmt
      ? (currency && !['INR', ''].includes(currency)
          ? formatDisplayMoney(Number(finalAmt), currency)
          : `&#8377;${Number(finalAmt).toLocaleString('en-IN')}`)
      : '';
    const preferredPay = String(f.preferredPaymentMethod ?? '').trim();
    const finalPay = String(f.paymentMethod ?? f.payoutMethod ?? '').trim();
    const paymentChanged =
      preferredPay.length > 0 &&
      finalPay.length > 0 &&
      preferredPay.toLowerCase() !== finalPay.toLowerCase();
    const returnMode = String(f.returnMode ?? 'store').trim().toLowerCase();
    ...
    const summaryRows: [string, string][] = [
      ['Request ID', String(f.requestId ?? '').trim()],
      ['Item', String(f.deviceName ?? '').trim()],
      ['Return option', isCourier ? 'Courier delivery' : 'Collect from our store'],
      ['Returned On', dv(String(f.returnedDate || f.completionDate || '').trim())],
      ...(isCourier && deliveryAddress
        ? [['Delivery address', deliveryAddress] as [string, string]]
        : !isCourier && storeAddress
          ? [['Pick up address', storeAddress] as [string, string]]
          : []),
      ...(displayAmt ? [['Final Amount', displayAmt] as [string, string]] : []),
      ...(finalPay || preferredPay
        ? [['Payment Method', finalPay || preferredPay] as [string, string]]
        : []),
    ].filter((row): row is [string, string] => String(row[1] ?? '').trim() !== '');
    const closing = String(f.closingMessage ?? '').trim();
    const noteBlock = closing
      ...
      : '';
    const paymentNoteBlock = paymentChanged
      ? alertBox(
          `You selected <strong>${escapeHtmlEntities(preferredPay)}</strong> when accepting the quote. Payment was received via <strong>${escapeHtmlEntities(finalPay)}</strong> at return.`,
          'info',
        )
      : preferredPay && finalPay && !paymentChanged
        ? alertBox(`Payment received via ${escapeHtmlEntities(finalPay)} as planned.`, 'success')
        : '';
    const completeMsg = isCourier
      ? 'We hope you have received your device. Your repair journey is complete — thank you for trusting Rekart.'
      : 'Your repair journey is complete. We hope your device is working perfectly — thank you for trusting Rekart.';
    return [
      highlight('Repair Complete', '&#127881;', 'Thank you for choosing Rekart'),
      noteBlock,
      infoTable('Summary', summaryRows),
      paymentNoteBlock,
      alertBox(completeMsg, 'success'),
    ]
      .filter(Boolean)
      .join('');
  },
  ...
  'courier-dispatched': (f, cb, t) => {
    ...
    return [
      highlight('Device Shipped', '&#128230;', 'Your repaired device is on the way'),
      alertBox(
        'Your repaired device has been dispatched via courier to the address you provided.',
        'info',
      ),
  ...
  'device-return-unrepaired': (f, cb, t) => {
    const device = String(f.deviceName ?? '').trim();
    ...
    return [
      highlight('Device Return', '&#128230;', 'Your device will be sent back without repair'),
      alertBox(
        'You chose not to proceed with the repair at our final quoted price. We will return your device in its current condition.',
        'info',
      ),
  ...
  'device-return-unrepaired-complete': (f) => {
    const device = String(f.deviceName ?? '').trim();
    const returnedOn = dv(String(f.returnedDate ?? f.completionDate ?? '').trim());
    return [
      highlight('Return Complete', '&#9989;', 'Your device has been returned'),
      alertBox(
        'Your device has been returned without repair. No repair charges apply for this request.',
        'success',
      ),
      infoTable('Summary', [
        ['Request ID', String(f.requestId ?? '').trim()],
...
(WORKFLOW_CONTENT as Record<string, WorkflowBuilderFn>)['booking-confirmed-reminder'] =
  (f, cb, t, ov) =>
    reminderBanner(REMINDER_MESSAGES['booking-confirmed-reminder'], f) +
    (WORKFLOW_CONTENT['booking-confirmed']!(f, cb, t, ov));

(WORKFLOW_CONTENT as Record<string, WorkflowBuilderFn>)['quote-ready-reminder'] =
  (f, cb, t, ov) =>
    reminderBanner(REMINDER_MESSAGES['quote-ready-reminder'], f) +
    (WORKFLOW_CONTENT['quote-ready']!(f, cb, t, ov));
...
(WORKFLOW_CONTENT as Record<string, WorkflowBuilderFn>)['device-ready-reminder'] =
  (f, cb, t, ov) =>
    reminderBanner(
      'Your device is still ready — please choose collect from store or courier delivery using the buttons below.',
      f,
    ) +
    (WORKFLOW_CONTENT['device-ready']!(f, cb, t, ov));
...
const REMINDER_MESSAGES: Record<string, string> = {
  'booking-confirmed-reminder':
    "We haven't heard back about your repair booking yet. Please use Schedule Pickup or Decline below so we can move forward &mdash; it only takes a second.",
  'quote-ready-reminder':
    "Your quote is still waiting! We've made you an offer for your device and would love to hear back. Please review and accept or decline using the buttons below.",
...
const STEP_NAMES: Record<string, string> = {
  ...
  'booking-confirmed-reminder': 'Request Received',
  'pickup-scheduled':          'Pickup Scheduled',
  'pickup-scheduled-reminder': 'Pickup Scheduled',
  'device-received':       'Device Received',
  'diagnosing':            'Diagnosing',
  'quote-ready':               'Quote Ready',
  'quote-ready-reminder':      'Quote Ready',
  'repair-in-progress':              'Repair In Progress',
  'device-ready':                    'Device Ready',
  completed:                   'Completed',
  'device-returned':             'Repair Complete',
  'courier-dispatched':          'Courier Dispatched',
};

const GREETINGS: Record<string, (f: FieldData) => string> = {
  'booking-confirmed':          (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'booking-confirmed-reminder': (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'pickup-scheduled':          (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'pickup-scheduled-reminder': (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'device-received':       (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'diagnosing':            (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'device-ready':          (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'quote-ready':               (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'quote-ready-reminder':      (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'repair-in-progress':              (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  completed:                   (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'device-returned':             (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
  'courier-dispatched':          (f) => `Hello${f.customerName ? `, ${f.customerName}` : ''},`,
};

const BODY_TEXT: Record<string, (f: FieldData) => string> = {
  'booking-confirmed': (f) =>
    `Thank you for booking a repair with Rekart. We've received your request. Please confirm using the button below to move to the next step.`,

  'pickup-scheduled': () =>
    `Great news - your pickup is confirmed. Please have your device and a valid ID ready. Our agent will contact you before arriving.`,

  'device-received': () =>
    `Your item has arrived safely at our service centre. Our technicians will begin diagnosis shortly.`,

  'diagnosing': () =>
    `Our technicians are diagnosing your item. We'll share a repair quote as soon as the assessment is complete.`,

  'device-ready': () =>
    `Great news — your item is ready. Choose collect from store or courier delivery using the buttons in this email.`,

  'quote-ready': () =>
    `We've assessed your item and prepared a repair quote. Review the amount below and let us know your decision.`,

  'booking-confirmed-reminder': () =>
    `Just a gentle nudge — your repair booking is waiting for your confirmation. Please use the button below at your earliest convenience.`,

  'pickup-scheduled-reminder': (f) => {
    const slot = pickupTimeDisplay(f);
    const day  = dv(f.pickupDate) || 'the scheduled date';
    return `This is a reminder about your upcoming pickup on ${day}${slot ? ` (${slot})` : ''}. Please ensure your item and a valid ID are ready when our agent arrives.`;
  },

  'quote-ready-reminder': () =>
    `Your repair quote is still open and waiting for your response. Accept to proceed with the repair, or decline if you've changed your mind.`,

  'repair-in-progress': () =>
    `Repair work has started on your item. We'll notify you when it's ready.`,

  completed: () =>
    `Your repair is now fully complete. It's been a pleasure doing business with you. We look forward to welcoming you back to Rekart.`,

  'device-returned': () =>
    `Your repair is complete and your item has been returned. Thank you for choosing Rekart — we hope everything works perfectly.`,

  'courier-dispatched': () =>
    `Your repaired device has been handed to the courier and is on its way to the delivery address you provided.`,
};

// ?? Main render function ??????????????????????????????????????????????????????
export function renderRepairEmailHtml(
  workflowKey: string,
  dynamicValues: Record<string, unknown>,
  opts: {
    workspaceName?: string;
  ...
  const contentFn = WORKFLOW_CONTENT[workflowKey] ?? WORKFLOW_CONTENT['device-returned'];
