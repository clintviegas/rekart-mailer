...
import { IsNumber, IsOptional, IsString, Min, Max, IsIn, MinLength, MaxLength, Matches } from 'class-validator';
import { Type } from 'class-transformer';
import { REPAIR_RETURN_PAYMENT_METHODS } from './repair-payment-methods';
import { REPAIR_PICKUP_TIME_WINDOW_SELECT_OPTIONS } from './repair-pickup-time-windows';

class BookingAcceptSubmitDto {
  @IsString()
  @IsIn([...REPAIR_PICKUP_TIME_WINDOW_SELECT_OPTIONS])
...
class QuoteAcceptDto {
  @IsString()
  @IsIn([...REPAIR_RETURN_PAYMENT_METHODS])
...
@Public()
@Throttle({ public: {} })
@Controller('repair/public')
export class RepairPublicActionController {
  constructor(private readonly service: RepairPublicActionService) {}
