...
import {
  ...
  DeliveryStatus,
} from '../modules/recycle/schemas/recycle-delivery-log.schema';
...
} from '../modules/recycle/templates/recycle-email-html.renderer';
import { createEmailProvider, FailoverEmailProvider } from '../providers/email/email-provider.factory';
import { TrackingTokenService } from '../modules/recycle/tracking-token.service';
import { UnsubscribeTokenService } from '../modules/recycle/unsubscribe-token.service';
import { RecycleSuppressionService } from '../modules/recycle/recycle-suppression.service';

@Injectable()
export class RecycleEmailWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(RecycleEmailWorker.name);
  ...
  onModuleInit() {
    if (process.env.Recycle_USE_BULL_QUEUE !== 'true') {
      this.logger.log(
        '[Worker] Recycle-email worker disabled (set Recycle_USE_BULL_QUEUE=true to process queued jobs)',
      );
      return;
    }
    ...
    this.worker = new Worker(
      RECYCLE_EMAIL_QUEUE,
      (job: Job<RecycleEmailJobPayload>) => this.processJob(job),
      {
    ...
    this.logger.log('[Worker] Recycle-email worker started');
  }

  async onModuleDestroy() {
    await this.worker?.close();
    this.logger.log('[Worker] Recycle-email worker stopped');
  }

  private async processJob(job: Job<RecycleEmailJobPayload>): Promise<void> {
    const {
      deliveryLogId,
      workspaceId,
      workflowKey,
    ...
    try {
      ...
      const trackingBase = `${backendBase}/api/v1/Recycle/track`;
      ...
      const unsubscribeUrl = `${backendBase}/api/v1/Recycle/unsubscribe/${unsubToken}`;

      // ── 6. Render HTML ────────────────────────────────────────────────────
      const html = renderRecycleEmailHtml(workflowKey, dynamicFieldValues, {
        workspaceName,
        logoUrl,
        mascotSrc,
        socialIconSrcs,
