...
const ATTACHMENTS_DIR = path.join(process.cwd(), 'uploads', 'repair-attachments');
import {
  RepairDeliveryLog,
  RepairDeliveryLogDocument,
  DeliveryStatus,
} from '../modules/repair/schemas/repair-delivery-log.schema';
...
} from '../modules/repair/templates/repair-email-html.renderer';
import { createEmailProvider, FailoverEmailProvider } from '../providers/email/email-provider.factory';
import { TrackingTokenService } from '../modules/repair/tracking-token.service';
import { UnsubscribeTokenService } from '../modules/repair/unsubscribe-token.service';
import { RepairSuppressionService } from '../modules/repair/repair-suppression.service';

@Injectable()
export class RepairEmailWorker implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(RepairEmailWorker.name);
  ...
  constructor(
    ...
    private readonly suppressionService: RepairSuppressionService,
  ...
  onModuleInit() {
    if (process.env.REPAIR_USE_BULL_QUEUE !== 'true') {
      this.logger.log(
        '[Worker] repair-email worker disabled (set REPAIR_USE_BULL_QUEUE=true to process queued jobs)',
      );
      return;
    }
    ...
    this.worker = new Worker(
      REPAIR_EMAIL_QUEUE,
      (job: Job<RepairEmailJobPayload>) => this.processJob(job),
      {
    ...
    this.logger.log('[Worker] repair-email worker started');
  }

  async onModuleDestroy() {
    await this.worker?.close();
    this.logger.log('[Worker] repair-email worker stopped');
  }

  private async processJob(job: Job<RepairEmailJobPayload>): Promise<void> {
    const {
      deliveryLogId,
      workspaceId,
      workflowKey,
    ...
    try {
      ...
      const trackingBase = `${backendBase}/api/v1/repair/track`;
      ...
      const unsubscribeUrl = `${backendBase}/api/v1/repair/unsubscribe/${unsubToken}`;

      // ── 6. Render HTML ────────────────────────────────────────────────────
      const html = renderRepairEmailHtml(workflowKey, dynamicFieldValues, {
        workspaceName,
        logoUrl,
        mascotSrc,
        socialIconSrcs,
